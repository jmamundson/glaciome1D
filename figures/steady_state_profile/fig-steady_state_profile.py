...
from glaciome1D import constants, glaciome, pressure, transverse, deformational_thickness
...
if run_simulations == 'y':
    ...
    if os.path.isfile('steady-state_Bdot_-0.6.pickle'):
        file = open('steady-state_Bdot_-0.6.pickle', 'rb')
        ...
        data.refine_grid(21)
        data.steadystate()
        data.save('steady-state_Bdot_-0.6.pickle')
    
    data.param.muW_max = 100
    data.steadystate()
    data.save('steady-state_Bdot_-0.6_noslip.pickle')
...
def plot_figure(data, axes, color_id, linestyle):
    ...
    for j in np.arange(0,3):#len(Hd)):
        ...
        u_slip = U[j]-u_mean#np.mean(u_transverse)
        u_transverse += u_slip
        ...
        ax5.legend(('$\chi=0.05$','$\chi=0.50$','$\chi=0.95$'))
...
ax2.plot(np.array([L,15000])*1e-3,np.array([0,0]),'k')
