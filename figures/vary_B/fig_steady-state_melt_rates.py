...
from glaciome1D import glaciome, pressure, deformational_thickness, force, constants
...
if run_simulations == 'y':
    ...
    data.refine_grid(21)
    data.steadystate()
    data.save('steady-state_Bdot_' + "{:.1f}".format(B_days[0]) + '.pickle')
...
ax2.set_xticks(np.linspace(0.3,0.9,7,endpoint=True))
ax2.set_xticklabels(['0.3','0.4','0.5','0.6','0.7','0.8','0.9'])
ax2.set_ylim([0.1,10])
...
bounds = cbar_ticks
norm = matplotlib.colors.Normalize(vmin=0.4, vmax=0.7)
